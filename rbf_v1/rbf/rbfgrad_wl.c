/* Gradient routine for rbf networks _with_ sample and label weightings
...
{
  double *dif_oy_etha_theta=(double *)calloc(b*l, sizeof(double)) ;
  ...
  for (j=0; j<b; j++)
    for (i=0; i<l; i++)
	 dif_oy_etha_theta[j+i*b]=(O[j+i*b]-Y[j+i*b])*eta[i]*theta[j+i*b] ;
         /* now we have dif_oy_etha_theta(j,i)= ...
            \gamma_j^{(i)}*(f_j^{(i)}-{\hat y}_j^{(i)}) */
  ...
  for (q=0; q<m; q++)
  {
	...
	double sum=0 ;
	for (i=0; i<l; i++)
	  sum = sum + dif_oy_etha_theta[j+i*b] * h_q__mu_q[i] ;
          /* now we have sum*w(j,q)= ...
	     \sum_{i=1}^l \gamma_j^{(i)}*(f_j^{(i)}-{\hat y}_j^{(i)}) * ...
    ...
    for (j=0; j<b; j++)
    {
      ...
      double sum=0 ;
      for (i=0; i<l; i++)
	sum = sum + dif_oy_etha_theta[j+i*b] * h_q__sigma_q[i] ;
        /* now we have sum*w(j,q)= ...
           \sum_{i=1}^l \gamma_j^{(i)}*(f_j^{(i)}-{\hat y}_j^{(i)}) * ...
...
{
  double *X, *Y, *O, *mu, *sigma, *w, *H, *Inp, *theta, *eta,
  ...
  theta=mxGetPr(prhs[8]) ; assert(theta!=0) ;
  /*printf("(%i,%i)\n", mxGetM(prhs[8]), mxGetN(prhs[8])) ;*/
  eta=mxGetPr(prhs[9]) ; assert(eta!=0) ;
  /*printf("(%i,%i)\n", mxGetM(prhs[9]), mxGetN(prhs[9])) ;*/
  h1_sigma_sigma=mxGetPr(prhs[10]) ; assert(h1_sigma_sigma!=0) ;
  /*printf("(%i,%i)\n", mxGetM(prhs[10]), mxGetN(prhs[10])) ;*/
  h2_sigma_sigma=mxGetPr(prhs[11]) ; assert(h2_sigma_sigma!=0) ;
  /*printf("(%i,%i)\n", mxGetM(prhs[11]), mxGetN(prhs[11])) ;*/
  dif_h1_h1=mxGetPr(prhs[12]) ; assert(dif_h1_h1!=0) ;
  /*printf("(%i,%i)\n", mxGetM(prhs[12]), mxGetN(prhs[12])) ;*/
  dif_h2_h2=mxGetPr(prhs[13]) ; assert(dif_h2_h2!=0) ;
  ...
  if ((mxGetM(prhs[8])!=b) | (mxGetN(prhs[8])!=l))
  {
    mexErrMsgTxt("Dimension error (arg 9).");
    return ;
  } ;
  if ((mxGetM(prhs[9])!=1) | (mxGetN(prhs[9])!=l))
  {
  ...
  if ((mxGetM(prhs[13])!=1) | (mxGetN(prhs[13])!=m))
  {
    mexErrMsgTxt("Dimension error (arg 14).");
    return ;
  } ;
  ...
  rbfgrad_wl(X, Y, O, mu, sigma, w, H, Inp, theta, eta,
