...
rbfgrad_exp_w(double *X, double *Y, double *O, double *mu, double *sigma, 
	  ...
	  double *C_mu, double *C_sigma, double *PatE)
{
  ...
  for (i=0; i<l; i++)
    dif_oy_etha[i]=-eta[i]*Y[i]*PatE[i] ;
    /* now we have dif_oy_etha(i)= ...
       -\eta^{(i)}*exp( -f^{(i)}*{\hat y}^{(i)} )*{\hat y}^{(i)} */
  ...
  for (q=0; q<m; q++)
  {
	...
	C__mu_q_s = C__mu_q_s + dif_oy_etha[i] * h_q__mu_q[i] ;
        /* now we have sum*w(q)= ...
	   \sum_{i=1}^l - \eta^{(i)}*exp(-f^{(i)}*{\hat y}^{(i)}) * {\hat y}^{(i)} *...
                        \frac{\partial}{\partial \mu_s^{[q]}} f^{(i)} */
      C__mu_q_s *= w[q] ; 
    
      C_mu[s+q*a]=2*C__mu_q_s ;
      /* now we have C_mu(s,q)= 2 * \sum_{i=1}^l ...
	 -\eta^{(i)}*exp(-f^{(i)}*{\hat y}^{(i)}) * {\hat y}^{(i)} * ...
	 \frac{\partial}{\partial \mu_s^{[q]}} f^{(i)} */
    } ;
    ...
    for (i=0; i<l; i++)
      ...
      /* now we have sum*w(q)= ...
           \sum_{i=1}^l - \eta^{(i)}*exp(-f^{(i)}*{\hat y}^{(i)}) * {\hat y}^{(i)} * ...
           \frac{\partial}{\partial \sigma^{[q]}} f^{(i)} */
    
    C__sigma_q *= w[q] ; 
  
    C_sigma[q]=2*C__sigma_q ;
    /* now we have C_sigma(q)= 2 * \sum_{i=1}^l ...
       -\eta^{(i)}*exp(-f^{(i)}*{\hat y}^{(i)}) * {\hat y}^{(i)} * ...
       \frac{\partial}{\partial \sigma^{[q]}} f^{(i)} */
  } ;
  free(dif_oy_etha) ;
    ...
    mexFuncConst Matrix *prhs[])
{
  ...
  double *C_mu=NULL, *C_sigma=NULL, *PatE ;

  if ((nrhs!=14) || (nlhs!=2))
  {
    mexErrMsgTxt("[C_mu, C_sigma]=rbfgrad_exp_w(X, Y, O, mu, sigma, w, H, Inp, eta, h1_sigma_sigma, h2_sigma_sigma, dif_h1_h1, dif_h2_h2, PatE)") ;
    return ;
  } ;
  ...
  PatE=mxGetPr(prhs[13]) ; assert(PatE) ;
  /*printf("(%i,%i)\n", mxGetM(prhs[12]), mxGetN(prhs[12])) ;*/
  ...
  rbfgrad_exp_w(X, Y, O, mu, sigma, w, H, Inp, eta,
	        ...
	        dif_h2_h2, a, m, l, C_mu, C_sigma, PatE) ;
} 
