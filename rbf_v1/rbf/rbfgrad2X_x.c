...
void 
rbfgrad2X_x(double *X, double *mu, double *sigma, 
	  double *H, double *w, unsigned int a, 
    ...
    mexFuncConst Matrix *prhs[])
{
  ...
  if ((nrhs!=5) || (nlhs!=1))
  {
    mexErrMsgTxt("C_X=rbfgrad2X_mu(X, mu, sigma, H, w)") ;
    return ;
  } ;
  ...
  if ((mxGetM(prhs[4])!=m) | (mxGetN(prhs[4])!=1))
  {
    mexErrMsgTxt("Dimension error (arg 5).");
    return ;
  } ;

  plhs[0]=(mxArray*)mxCreateDoubleMatrix(1, l, mxREAL) ; 
  C_X = mxGetPr(plhs[0]);
    
  rbfgrad2X_x(X, mu, sigma, H, w, a, m, l, C_X) ;
} 
