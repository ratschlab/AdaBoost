...
void *ht_h_lambda_thread_func(void * P) 
{
...
void ht_h_lambda(double * x, double lambda, double *z, unsigned int n, unsigned int m)
{
  ...
  if (n>PAR_THRESH)
  {
    ...
    for (k=0; k<NUM_CPUS; k++)
    {
      ...
      thr_create(NULL, 0, ht_h_lambda_thread_func, (void*)&TD[k], PTHREAD_SCOPE_SYSTEM, &tid[k]) ;
    } ;
  ...
  } else
  {
    ...
    TD[0].js=0 ; TD[0].je=n ;
    ht_h_lambda_thread_func((void *)&TD[0]) ;
  } ;
